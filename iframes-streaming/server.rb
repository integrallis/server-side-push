...
get '/long-running' do   
  stream do |out|
    # padding to circumvent IE's buffer (damned you IE!!!!)
    out << ('*' * 256)  
    
    (10..100).step(10) do |n| 
      word = gimme_funny_word
      out << update_progress(n, word.keys.first, word.values.first) 
      sleep 1.5      
    end
  end
end   
...
def update_progress(percent, word, meaning)
  %[<script type="text/javascript">parent.updatePage(#{percent}, '#{word}', '#{meaning}');</script>]
end  
